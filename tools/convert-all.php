...
foreach ($rows as $r) {
    ...
    $nb = $content==='' ? 0 : substr_count($content,'wp:')/2;
    if ($DRY) { printf("  [DRY] #%d %s/%s \"%s\" → %dB\n",$r->ID,$r->post_type,$r->post_status,$r->post_title,strlen((string)$content)); continue; }
    // 1) nastav nativní obsah
    wp_update_post(array('ID'=>$r->ID,'post_content'=>(string)$content));
    // 2) smaž všechna _elementor_* meta
    $deleted = $wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->postmeta} WHERE post_id=%d AND meta_key LIKE %s", $r->ID, '_elementor%'));
    printf("  ✓ #%d \"%s\" → %dB obsah, %d elementor-meta smazáno\n",$r->ID,$r->post_title,strlen((string)$content),$deleted);
}
